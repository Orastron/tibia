...
static void plugin_process(plugin *instance, const float **inputs, float **outputs, size_t n_samples) {
	...
	for (size_t i = 0; i < n_samples; i++) {
		...
		if (instance->delay_line_cur == instance->delay_line_length)
			instance->delay_line_cur = 0;
		float y = x + mA1 * (instance->z1 - x);
		instance->z1 = y;
		instance->phase += phase_inc;
		if (instance->phase > 1.f)
			instance->phase -= 1.f;
		const float lfo = instance->phase + instance->phase - 1.f;
		y *= 1.f + kt * lfo;
...
static int plugin_state_save(plugin *instance, const plugin_state_callbacks *cbs, float last_sample_rate) {
	...
	const float tremolo = instance->tremolo;
